...
- (SudokuCell*)cellAtRow:(unsigned int)row andCol:(unsigned int)col
{
...
- (void)layoutSubviews
{
    ...
    const unsigned int usableWidth = ((unsigned int) rect.size.width);
    const unsigned int cellWidth = (unsigned int) floorf(usableWidth/NCOLS);
...
- (IBAction)buttonTapped:(SudokuCell*)sender
{
    ...
    [_delegate setSelectedCellAtIndex:sender.cellContentPtr->m_index];
}
